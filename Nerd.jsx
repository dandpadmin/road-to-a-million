...
function EfficiencyScatter({ data }) {
  ...
  if (!pts.length) {
    return (
      <div style={{ border: '1px dashed rgba(246,240,227,.22)', padding: 40, textAlign: 'center', fontSize: 11, letterSpacing: '.18em', textTransform: 'uppercase', color: 'rgba(246,240,227,.35)' }}>Awaiting drives with temperature data</div>
    );
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <div style={{ border: '1px solid rgba(246,240,227,.18)' }}>
        <svg viewBox={'0 0 ' + W + ' ' + H} preserveAspectRatio="none" style={{ display: 'block', width: '100%', height: 'auto', aspectRatio: W + ' / ' + H }}>
          {ys.ticks.map((v, i) => (
            <g key={'y' + i}>
              <line x1={L} y1={py(v)} x2={W - R} y2={py(v)} stroke="rgba(246,240,227,.08)" strokeWidth="1" />
          ...
          {pts.map((p, i) => (
            <circle key={i} cx={px(p.t)} cy={py(p.wh)} r={3} fill={provColor(provinces, p.prov)} fillOpacity="0.75">
      ...
      <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center', fontSize: 10, letterSpacing: '.16em', textTransform: 'uppercase', color: AXIS_TEXT }}>
        ...
        <span style={{ marginLeft: 'auto', color: 'rgba(246,240,227,.35)' }}>Outside temperature °C · {pts.length} drives</span>
      </div>
    </div>
  );
}
...
function ChargeCurve({ data }) {
  ...
  if (pts.length < 2) {
    return (
      <div style={{ border: '1px dashed rgba(246,240,227,.22)', padding: 34, textAlign: 'center', fontSize: 11, letterSpacing: '.18em', textTransform: 'uppercase', color: 'rgba(246,240,227,.35)' }}>Curve fills in as sessions accumulate — {pts.length} of the 2 points needed</div>
    );
  }
  ...
  return (
    <div style={{ border: '1px solid rgba(246,240,227,.18)' }}>
      <svg viewBox={'0 0 ' + W + ' ' + H} preserveAspectRatio="none" style={{ display: 'block', width: '100%', height: 'auto', aspectRatio: W + ' / ' + H }}>
        ...
        {pts.map((p, i) => (
          <circle key={i} cx={px(p.soc)} cy={py(p.kw)} r={4} fill="#1C242C" stroke="#00B4D9" strokeWidth="2">
...
function NerdStat({ label, value, unit, sub }) {
  return (
    <Plate tone="ghost" pad={22} style={{ gap: 12, alignContent: 'start' }}>
      <Eyebrow tone="bronze" size={10} track={0.22}>{label}</Eyebrow>
      <StatReadout value={value} unit={unit || ''} size={34} />
      {sub && <div style={{ fontSize: 10, letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(246,240,227,.5)' }}>{sub}</div>}
    </Plate>
  );
}
...
function FirmwareLog({ versions }) {
  ...
  if (!list.length) {
    return <div style={{ fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(246,240,227,.35)' }}>Recording from today — no changes seen yet</div>;
  }
  return (
    <div style={{ display: 'grid', gap: 0 }}>
      {list.map((v, i) => (
        <div key={v.version + i} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, padding: '11px 0', borderBottom: '1px solid rgba(246,240,227,.10)' }}>
          <span style={{ fontSize: 12, fontVariantNumeric: 'tabular-nums', color: i === 0 ? '#F6F0E3' : 'rgba(246,240,227,.6)' }}>{v.version}</span>
          <span style={{ fontSize: 10, letterSpacing: '.16em', textTransform: 'uppercase', color: 'rgba(246,240,227,.45)' }}>{i === 0 ? 'Current · from ' : 'From '}{v.since}</span>
        </div>
      ))}
    </div>
  );
